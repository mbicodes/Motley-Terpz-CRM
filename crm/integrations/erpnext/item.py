...
def get_item_price_rate(item_code: str, uom: str | None = None):
	...
	# Never let a pricing lookup abort the Item save that triggered this sync.
	try:
	...
	if not rows:
		return None
	row = rows[0]
	# ERPNext version differences: get_item_price may return dicts (frappe._dict)
	# or plain result tuples in the shape (name, price_list_rate, uom).
	if isinstance(row, dict):
		return row.get("price_list_rate")
	try:
		return row[1]
